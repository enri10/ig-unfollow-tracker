...
// After this many *consecutive* 429s, stop the automatic short-interval
// resume-alarm loop entirely and fall back to the once-daily alarm — see the
// long comment where this is used, in performCheck's catch block.
export const RATE_LIMIT_CIRCUIT_BREAKER_THRESHOLD = 3;

export async function ensureDailyAlarm() {
  const settings = await storage.getSettings();
  if (settings.checksPaused) return; // respect an existing pause across restarts/reinstalls
...
/**
 * Fully stop (or resume) all automatic checking — not just backing off, but
 * zero requests going out at all until explicitly resumed. Added after a
 * real incident where daily automatic retries kept hitting a 429 for 9+
 * days straight: the circuit breaker (see RATE_LIMIT_CIRCUIT_BREAKER_THRESHOLD)
 * only slows automatic retries down to once a day, but even that is still an
 * automatic request against an account that may need a longer, completely
 * uninterrupted rest — this gives the user (or the popup UI) a way to
 * actually guarantee that.
 */
export async function setChecksPaused(paused) {
  await storage.setSettings({ checksPaused: paused });
  if (paused) {
...
export async function performCheck({ manual = false } = {}) {
  ...
  // A full stop, not just a slowdown: while paused, not even a manual
  // "Check now" click goes through. The circuit breaker above only reduces
  // automatic retries to once a day; this is for when even that isn't
  // appropriate and the account genuinely needs zero requests for a while.
  const settings0 = await storage.getSettings();
  if (settings0.checksPaused) {
    await storage.logAttempt({ outcome: "skipped", reason: "paused", manual });
    return { skipped: true, reason: "paused" };
  }

  if (state.isCheckRunning && !isRunStale(state)) {
    await storage.logAttempt({ outcome: "skipped", reason: "already-running", manual });
  ...
  if (!state.partialRun) {
    ...
    if (sinceLast < minGapMs) {
      await storage.logAttempt({ outcome: "skipped", reason: "too-soon", manual });
  ...
  try {
    ...
    const settings = await storage.getSettings();
    let partialRun = state.partialRun || null;
    // A following-stage checkpoint from before trackFollowing was turned off
    // no longer applies — there's nothing to resume into.
    if (partialRun && partialRun.stage === "following" && !settings.trackFollowing) {
      partialRun = null;
    }
    ...
    // --- Following (opt-in — see settings.trackFollowing default in storage.js) ---
    let following = [];
    if (settings.trackFollowing) {
    ...
    // Both lists (or just followers, if that's all we're tracking) are complete.
    ...
    const snapshot = {
      ...
      // When following isn't tracked, `following` is always [] — fall back
      // to Instagram's own reported count so the popup still shows a real
      // number instead of a misleading 0.
      followingCount: settings.trackFollowing ? following.length : reportedFollowingCount ?? 0,
    ...
    const followingSinceMap = settings.trackFollowing
      ? await updateFollowingSince(following, snapshot.takenAt)
      : {};
    ...
    await storage.logAttempt({ outcome: "success", manual, lostFollowerCount: diff.lostFollowers.length });
    ...
    await storage.logAttempt({ outcome: "error", errorType: errorPayload.type, manual });
    ...
    if (errorPayload.type === "RATE_LIMITED") {
      const rateState = await storage.getState();
      const streak = (rateState.rateLimitStreak || 0) + 1;

      if (streak >= RATE_LIMIT_CIRCUIT_BREAKER_THRESHOLD) {
        // Backing off exponentially and still getting 429s this many times
        // in a row means Instagram's actual cooldown on this account is
        // longer than our schedule assumes (this happened for real: it
        // recurred a full day later). Retrying automatically at ANY
        // interval at that point risks looking like exactly the automated
        // abuse pattern that got the account flagged in the first place —
        // so stop the automatic resume loop entirely and defer to the
        // once-daily alarm instead. Manual "Check now" still works; that's
        // the user's own informed choice, not an automatic retry loop.
        await storage.setState({ rateLimitStreak: streak, nextRetryAt: null, autoRetryPaused: true });
      } else {
        // Back off exponentially rather than always retrying after a fixed
        // 15 minutes: a 429 means Instagram is actively throttling this
        // session, and retrying before that cools down just extends the
        // block. Consecutive 429s push the wait out further; any success
        // resets the streak above.
        ...
        await storage.setState({ rateLimitStreak: streak, nextRetryAt, autoRetryPaused: false });
