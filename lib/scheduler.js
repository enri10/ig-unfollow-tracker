...
import { todayId, randomDelay } from "./utils.js";
 ...
 */
async function fetchListResumable({ userId, username, stage, collected, nextMaxId, completedFollowers }) {
    ...
    onCheckpoint: async (collectedSoFar, cursor) => {
      if (cursor) {
        await storage.setState({
          partialRun: { userId, username, stage, collected: collectedSoFar, nextMaxId: cursor, completedFollowers },
        });
      }
    },
  });
...
export async function performCheck({ manual = false } = {}) {
  ...
  try {
    ...
    if (partialRun && partialRun.stage === "followers") {
      ...
      followers = await fetchListResumable({ userId, username, stage: "followers", collected: [], nextMaxId: null });
    }
    ...
    if (errorPayload.type === "RATE_LIMITED") {
      chrome.alarms.create(RESUME_ALARM, { delayInMinutes: 15 });
    }
