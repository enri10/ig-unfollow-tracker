...
const DEFAULT_SETTINGS = {
  ...
  // A full stop: while true, performCheck refuses to run at all — not even
  // a manual "Check now" — and no alarm is scheduled. See
  // lib/scheduler.js's setChecksPaused. Off by default; the user turns this
  // on (from the popup or options) when the account needs a longer,
  // completely uninterrupted rest than the automatic backoff alone gives it.
  checksPaused: false,
};
...
const DEFAULT_STATE = {
  ...
  // Set once rateLimitStreak crosses the circuit-breaker threshold (see
  // lib/scheduler.js): automatic short-interval retries stop entirely rather
  // than keep escalating, deferring to the once-daily alarm instead. Manual
  // "Check now" still works (the user's informed choice), it's only the
  // automatic resume-alarm loop that halts.
  autoRetryPaused: false,
};

// Bounded log of every check *attempt* (success, skip, or error) — separate
// from the diff history in IndexedDB, which only ever records successful
// checks. This is what answers "did it even try, and what happened" when a
// check fails repeatedly and never produces a diff to show in History.
const MAX_ATTEMPT_LOG_ENTRIES = 20;
...
// ---------- Check-attempt log (visibility into every try, not just successes) ----------
...
/**
 * Record one check attempt, regardless of outcome.
 * @param {object} entry - e.g. { outcome: 'success'|'skipped'|'error', manual, reason?, errorType? }
 */
export async function logAttempt(entry) {
  const log = await getAttemptLog();
  log.unshift({ at: Date.now(), ...entry });
  await chrome.storage.local.set({ attemptLog: log.slice(0, MAX_ATTEMPT_LOG_ENTRIES) });
}
